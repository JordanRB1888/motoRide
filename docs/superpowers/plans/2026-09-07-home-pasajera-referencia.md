...
- **No se toca backend, lógica de negocio, contratos, auth ni navegación.** Las claves que la app real entiende (`crearNavegacionDePasajero`) son `inicio`, `seguro`, `historial`, `perfil`, `saldo`, `pedir`, `buscar-destino`, `servicio` (sólo `viajes`), `avisos` y `viaje-detalle`. **`comercios` NO existe en la app real** (cae en `default:` y no hace nada, como antes del rediseño); en el laboratorio lleva a la pantalla de comercios. Lo que no existe navega a `pronto` en el laboratorio y no hace nada en la app real, igual que hoy. Decisión pendiente del dueño: pantalla real de comercios/promociones o no pintar esas acciones en la app real.
