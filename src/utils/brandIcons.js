/** Inline provider marks. Kept local so the login does not contact third-party
 * hosts before the user explicitly chooses an authentication provider. */
const BRANDS = {
  google: `
...
export function brandIcon(name, size = 18) {
  ...
  if (!brand) return '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" aria-hidden="true" focusable="false" class="brand-icon brand-icon-${name}">${brand}</svg>`;
}
