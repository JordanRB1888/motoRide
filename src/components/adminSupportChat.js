...
export function createAdminSupportChat(user) {
  ...
  const load=async()=>{const conversationId=user?.id||'d1';const page=await apiService.get(`/support/threads/${encodeURIComponent(conversationId)}/messages?limit=50`);messages=Array.isArray(page?.items)?[...page.items].reverse():[];render();};
