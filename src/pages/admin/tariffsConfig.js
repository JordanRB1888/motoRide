...
export async function renderTariffsConfig(container) {
    ...
    container.innerHTML = `
      ...
      <form id="pricing-form" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px">
        <section class="diorama-card-3d" style="padding:22px;border-radius:22px;background:var(--surface-card)"><h3>🏍️ Mototaxi</h3><div style="display:grid;gap:13px">${field('m-base','Tarifa base USD',moto.baseFareUSD)}${field('m-km','Precio por km',moto.pricePerKmUSD)}${field('m-min','Precio por minuto',moto.pricePerMinuteUSD)}${field('m-minimum','Tarifa mínima',moto.minimumFareUSD)}</div></section>
        <section class="diorama-card-3d" style="padding:22px;border-radius:22px;background:var(--surface-card)"><h3>🚗 Automóvil</h3><div style="display:grid;gap:13px">${field('c-base','Tarifa base USD',car.baseFareUSD)}${field('c-km','Precio por km',car.pricePerKmUSD)}${field('c-min','Precio por minuto',car.pricePerMinuteUSD)}${field('c-minimum','Tarifa mínima',car.minimumFareUSD)}</div></section>
