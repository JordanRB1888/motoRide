...
export function renderDriverApplicationsManagement(container) {
  ...
  const render = () => {
    ...
    container.innerHTML = `<div class="driver-admin-page">
      ...
      <section class="driver-admin-table-card"><div class="ops-table-wrap"><table class="data-table"><thead><tr><th>Solicitante</th><th>Contacto</th><th>Vehículo</th><th>Enviada</th><th>Estado</th><th></th></tr></thead><tbody>${apps.map(app=>`<tr><td><div class="applicant-cell"><span>${escape(app.personal.firstName?.[0]||'C')}</span><div><strong>${escape(app.personal.firstName)} ${escape(app.personal.lastName)}</strong><small>${escape(app.personal.identityNumber)}</small></div></div></td><td><strong>${escape(app.personal.phone)}</strong><small class="table-subline">${escape(app.personal.email)}</small></td><td><strong>${app.vehicle.type==='CAR'?'Automóvil':'Moto'} · ${escape(app.vehicle.brand)}</strong><small class="table-subline">${escape(app.vehicle.plate)}</small></td><td>${new Date(app.submittedAt||app.createdAt).toLocaleDateString('es-VE')}</td><td><span class="application-status-badge ${app.status}">${STATUS[app.status]}</span></td><td><button class="review-application" data-review="${app.id}">${icon('eye',15)} Revisar</button></td></tr>`).join('') || '<tr><td colspan="6" class="empty-cell">No hay solicitudes con este filtro.</td></tr>'}</tbody></table></div></section>
