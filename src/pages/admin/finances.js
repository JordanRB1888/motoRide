...
export async function renderFinances(container) {
  ...
  const draw = () => {
    ...
    container.querySelectorAll('[data-transaction]').forEach(button => button.onclick = async () => {
      const result = await apiService.patch(`/admin/transactions/${button.dataset.transaction}`, { status: button.dataset.status, referenceConfirmed: button.dataset.type === 'TOP_UP' && button.dataset.status === 'APPROVED' });
      if (!result) return showToast('No se pudo conciliar la solicitud.', 'error');
      showToast('Movimiento conciliado.', 'success'); await reload();
    });
    container.querySelectorAll('[data-open-topups]').forEach(button => button.onclick = () => window.dispatchEvent(new CustomEvent('58express:admin-tab', { detail: 'topups' })));
  };
  draw();
}
