...
export function renderUsersManagement(container) {
  ...
  const detail = user => {
    ...
    return `<aside class="user-detail-drawer">
      <button id="close-user-detail" class="user-detail-close" type="button">${icon('close', 20)}</button>
      ...
      <section><h3>Contacto</h3><dl><div><dt>${icon('phone', 14)} Teléfono</dt><dd>${escape(user.phone || 'No registrado')}</dd></div><div><dt>${icon('message', 14)} Correo</dt><dd>${escape(user.email || 'No registrado')}</dd></div></dl></section>
      ...
      <section class="user-wallet-summary"><div><span>${icon('wallet', 20)}</span><p><small>Saldo disponible</small><strong>$${Number(user.walletBalance || 0).toFixed(2)}</strong></p></div><span>${Number(user.totalTrips || relatedTrips.length)} servicios totales</span></section>
  ...
  const render = () => {
    ...
    container.innerHTML = `<div class="users-command-view ${selected ? 'with-detail' : ''}">
      <div class="users-command-main">
        <header class="users-command-heading"><div><div><h1>Gestión de Usuarios</h1><span><i></i> Cuentas reales</span></div><p>Administra conductores y pasajeros registrados en +58Express.</p></div><button id="new-driver" type="button">${icon('plus', 16)} Nuevo conductor</button></header>
