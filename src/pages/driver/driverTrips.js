...
export function renderDriverTrips() {
  ...
  const openDetail = async trip => {
    ...
    const attachments = merged.filter(message => message.image);
    ...
    container.innerHTML = `<button class="trip-detail-back">${icon('back',17)} Volver a mis viajes</button>
      <section class="driver-trip-detail">
        ...
        <div class="trip-conversation"><h3>Conversación (${merged.length})</h3>${merged.length ? merged.map(message => `<div class="archived-message ${message.senderId === user.id ? 'mine' : ''}">${message.image ? `<img src="${esc(message.image)}" alt="Captura adjunta">` : ''}<p>${esc(message.text || 'Imagen adjunta')}</p><small>${date(message.timestamp || message.createdAt)}</small></div>`).join('') : '<p class="trip-detail-empty">No hubo mensajes durante este viaje.</p>'}</div>
        <div class="trip-attachments"><h3>Capturas y comprobantes (${attachments.length})</h3><div>${attachments.length ? attachments.map(item => `<a href="${esc(item.image)}" target="_blank" rel="noopener"><img src="${esc(item.image)}" alt="Comprobante del viaje"></a>`).join('') : '<p class="trip-detail-empty">No se adjuntaron capturas o comprobantes.</p>'}</div></div>
      </section>`;
    container.querySelector('.trip-detail-back').addEventListener('click', drawList);
  };
