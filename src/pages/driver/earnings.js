...
export function renderEarnings() {
  ...
  const render = () => {
    ...
    // El pago de una comisión pendiente es un débito real del saldo del
    // conductor: la octava auditoría encontró que ocurría de verdad y no
    // aparecía aquí, así que el saldo bajaba sin nada que lo explicara.
    /**
     * El saldo con el que quedó la cuenta después del movimiento.
     *
     * La novena auditoría lo encontró: la respuesta lo traía y la pantalla no
     * lo pintaba, así que un débito real —una comisión pendiente saldada— se
     * veía como una cifra suelta sin nada que la situara.
     *
     * Solo se muestra cuando el apunte lo trae de verdad. No se calcula ni se
     * deduce: un saldo inventado en un historial de dinero es peor que ninguno.
     */
    const saldoTras = item => Number.isFinite(Number(item?.balanceAfter))
      ? `<small class="earning-balance-after">Saldo: $${Number(item.balanceAfter).toFixed(2)}</small>`
      : '';

    const movements = (wallet.transactions || []).filter(item => ['DRIVER_EARNING','PLATFORM_COMMISSION','TOP_UP','PAYOUT','DRIVER_ACCOUNT_MAINTENANCE','DRIVER_DEFERRED_COMMISSION_PAYMENT'].includes(item.type)).slice(0,8);
    ...
    container.innerHTML = `<header class="earnings-title"><h2>Cuenta operativa</h2><span>Tasa referencial BCV: Bs. ${bcv.toFixed(2)}</span></header>
      ...
      <section class="earnings-activity-card"><div class="earnings-card-title compact"><span>${icon('clock',20)}</span><strong>Movimientos de la cuenta</strong></div><div class="earnings-activity-list">${movements.map(item=>{const debit=Number(item.amount)<0;const labels={DRIVER_EARNING:'Ganancia acreditada',PLATFORM_COMMISSION:'Comisión +58Express',TOP_UP:'Recarga',PAYOUT:'Liquidación',DRIVER_ACCOUNT_MAINTENANCE:'Mantenimiento de cuenta',DRIVER_DEFERRED_COMMISSION_PAYMENT:'Comisión pendiente saldada'};return `<div class="earning-row"><span class="earning-bike">${icon(['PLATFORM_COMMISSION','DRIVER_DEFERRED_COMMISSION_PAYMENT'].includes(item.type)?'dollarSign':'wallet',17)}</span><div><strong>${labels[item.type]||item.type}</strong><small>${item.tripId?`${paymentLabel(item.paymentMethod)} · Viaje #${item.tripId.slice(-6)}`:new Date(item.createdAt).toLocaleString('es-VE')}</small></div><div class="earning-row-amount ${debit?'debit':''}"><strong>${debit?'−':'+'}$${Math.abs(Number(item.amount||0)).toFixed(2)}</strong>${saldoTras(item)}</div><span class="earning-status">${item.status}</span></div>`;}).join('')||'<p class="wallet-empty">Todavía no hay movimientos.</p>'}</div></section>`;
