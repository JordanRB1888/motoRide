...
export function renderEarnings() {
  ...
  const openTopup = () => {
    ...
    modal.innerHTML = `<form class="wallet-topup-modal">
      ...
      <span>${icon('wallet',22)}</span><small class="wallet-modal-eyebrow">SALDO OPERATIVO DEL CONDUCTOR</small>
      <h3>Recargar Wallet</h3><p>Realiza el Pago Móvil, registra la referencia y administración validará la recarga.</p>
      <div class="wallet-payment-data"><div><small>BANCO</small><strong>${PAYMENT_DATA.bank}</strong></div><div><small>CÉDULA / RIF</small><strong>${PAYMENT_DATA.identity}</strong></div><div><small>TELÉFONO</small><strong>${PAYMENT_DATA.phone}</strong></div><button type="button" data-copy>${icon('copy',15)} Copiar datos</button></div>
  ...
  const render = () => {
    ...
    const movements = (wallet.transactions || []).filter(item => ['DRIVER_EARNING','PLATFORM_COMMISSION','TOP_UP','PAYOUT'].includes(item.type)).slice(0,8);
