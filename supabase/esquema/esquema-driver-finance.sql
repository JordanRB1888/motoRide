-- =============================================================================
--  +58Express — libro de comisiones del conductor (APARTE, y todavia no vive)
-- =============================================================================
--
--  QUE ES
--
--  Las tablas del control financiero del conductor: reservas de comision,
--  obligaciones de mantenimiento, estado financiero, avisos por inactividad y
--  operaciones de dinero.
--
--  POR QUE VA EN UN FICHERO APARTE Y NO EN EL ESQUEMA PRINCIPAL
--
--  Porque el CODIGO que las usa no esta fusionado. Vive en la rama
--  `feat/driver-finance-1`: 12 commits, 27 ficheros de servidor, ninguno de
--  movil. Mientras esa rama no entre en master, estas tablas se quedarian
--  vacias y sin nadie que las escriba.
--
--  HAY UNA INCOHERENCIA YA PUBLICADA, y conviene saberla: `/api/health` en
--  staging responde `driverCommissionDebtLedger: true`, anunciando una
--  capacidad cuyo codigo no esta desplegado. El endpoint promete algo que hoy
--  no existe.
--
--  CUANDO EJECUTARLO
--
--  El dia que se decida fusionar `feat/driver-finance-1`, y DESPUES del
--  esquema principal: estas tablas referencian `users` con claves foraneas.
--
--      psql "$DATABASE_URL" -f esquema-completo.sql
--      psql "$DATABASE_URL" -f esquema-driver-finance.sql
--
--  Generado desde `supabase/migrations/20260829180000_driver_finance_ledger.sql` en la rama `feat/driver-finance-1`.
-- =============================================================================
...
--
--  Igual que en el esquema principal: sin esto, `npm run db:migrate` la
--  aplicaria otra vez el dia que la rama se fusione.
-- =============================================================================

insert into public.schema_migrations (id, applied_at)
values ('20260829180000_driver_finance_ledger.sql', now())
