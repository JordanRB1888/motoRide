...
function applicationForm(email='+58driver@example.com',phone='+584120009999'){const form=new FormData();Object.entries({firstName:'María',lastName:'Conductora',identityNumber:'V-24680135',birthDate:'1994-05-20',phone,email,password:'ClaveSegura123',address:'Avenida principal, Maracaibo',city:'Maracaibo',region:'Zulia',vehicleType:'MOTO',vehicleBrand:'Bera',vehicleModel:'BR200',vehicleYear:'2024',vehicleColor:'Amarillo',vehiclePlate:'APP58X'}).forEach(([key,value])=>form.append(key,value));requiredDocs.forEach(type=>form.append(type,new Blob([png],{type:'image/png'}),`${type}.png`));return form;}
...
test('solicitud real: documentos privados, correcciones, auditoría y aprobación',async t=>{const api=await start(t);const admin=await login(api,'admin@58express.com','admin','admin');
  ...
  const createdResponse=await fetch(`${api}/driver-applications`,{method:'POST',body:applicationForm()});assert.equal(createdResponse.status,201);const created=await createdResponse.json();assert.equal(created.application.status,'pending');assert.equal(created.user.role,'passenger');assert.equal(created.application.documents.length,7);
  ...
  const privateDocument=created.application.documents[0];const ownerFile=await fetch(`${api}${privateDocument.contentUrl}`,{headers:{authorization:`Bearer ${created.token}`}});assert.equal(ownerFile.status,200);assert.equal(ownerFile.headers.get('cache-control'),'private, no-store, max-age=0');
  const strangerResponse=await fetch(`${api}/auth/register`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:'stranger@example.com',phone:'+584120008888',password:'ClaveSegura123',role:'passenger',firstName:'Otra',lastName:'Persona'})});const stranger=await strangerResponse.json();const stolen=await fetch(`${api}${privateDocument.contentUrl}`,{headers:{authorization:`Bearer ${stranger.token}`}});assert.equal(stolen.status,403);
  const adminFile=await fetch(`${api}${privateDocument.contentUrl}`,{headers:{authorization:`Bearer ${admin.token}`}});assert.equal(adminFile.status,200);
