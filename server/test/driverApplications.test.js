...
test('un pasajero existente puede solicitar ser conductor con su misma cuenta',async t=>{const api=await start(t);
  ...
  assert.equal(application.user.id,passenger.user.id);assert.equal(application.application.userId,passenger.user.id);assert.equal(application.application.status,'pending');
